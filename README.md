...
| Appendix B | SQL schema | `prisma/schema.prisma` (vector dim 768 vs proposal's 1536 — we use Google `text-embedding-004` which is 768-dim) |
...
Optional: `GEMINI_MODEL=gemini-2.5-flash` to roll forward when 2.0-flash is sunset.
...
- **Gemini 2.0/2.5 Flash** for the multimodal reasoning call (hosted, no training)
- **`text-embedding-004`** for policy embeddings (hosted, no training)
